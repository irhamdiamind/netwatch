const service = require("../services/device-category.service");
const {
  createDeviceCategorySchema,
  updateDeviceCategorySchema,
} = require("../validations/device-category.validation");
...
exports.create = async (req, res) => {
  const parsed = createDeviceCategorySchema.safeParse(req.body);
  if (!parsed.success)
...
exports.update = async (req, res) => {
  const parsed = updateDeviceCategorySchema.safeParse(req.body);
  if (!parsed.success)
    return res.status(400).json({ message: parsed.error.errors });
